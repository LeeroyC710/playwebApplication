...
class ApplicationUsingJsonReadersWriters @Inject()(components :ControllerComponents, authAction: AuthenticationAction, val reactiveMongoApi :ReactiveMongoApi )
  ...
  def create(comment: String): Action[AnyContent] = authAction.async { implicit request: Request[AnyContent] =>
    ...
    futureResult.map(_ => Redirect(routes.ApplicationUsingJsonReadersWriters.getAllEvents()))
  }

  def getAllEvents(): Action[AnyContent] = Action.async {
    ...
    futureUsersList.map { events =>
      //var listEvents :String = ""
      //posts.foreach( events => { listEvents += event + "\n" })
      //Ok(listEvents)
      Ok(views.html.view_all_events(events))
    }
  }
}